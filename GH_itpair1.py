...
class GH(object):
    ...
    def _build_model(self):
        ...
        # Update label network
        item_l = tf.matmul(self.ph['H'], tf.transpose(self.Hsh_L))
        self.Loss_l = self.mse_loss(tf.multiply(self.ph['Sim'], item_l), tf.log(1.0 + tf.exp(item_l))) + self.mse_loss(self.Lab_L, self.ph['L_batch'])
        # self.Loss_l = self.mse_loss(item_l, check*self.ph['Sim']) + self.mse_loss(self.Lab_L, self.ph['L_batch'])  # 改变相似度度量方式
        ...
        item_i = tf.matmul(self.Hsh_I, tf.transpose(self.Hsh_I))
        self.Loss_pair_i = self.mse_loss(tf.multiply(self.ph['Sim'], item_i), tf.log(1.0 + tf.exp(item_i)))
        # 加入不同的标签指示模块
        self.Loss_i2l = tf.matmul(tf.matmul(tf.transpose(self.Hsh_I), self.ph['Sim']), self.Hsh_I) - \
                        2 * tf.matmul(tf.matmul(tf.transpose(self.Hsh_I), self.ph['Sim']), self.ph['H']) + \
                        tf.matmul(tf.matmul(tf.transpose(self.ph['H']), self.ph['Sim']), self.ph['H'])
        self.Loss_i2l = tf.trace(self.Loss_i2l) / 250000
        self.Loss_I_hsh = self.Loss_pair_i + self.Loss_i2l + self.mse_loss(self.Hsh_I, self.ph['B'])  # J1
        ...
        item_t = tf.matmul(self.Hsh_T, tf.transpose(self.Hsh_T))
        self.Loss_pair_t = self.mse_loss(tf.multiply(self.ph['Sim'], item_t), tf.log(1.0 + tf.exp(item_t)))
        # 加入不同的标签指示模块
        self.Loss_t2l = tf.matmul(tf.matmul(tf.transpose(self.Hsh_T), self.ph['Sim']), self.Hsh_T) -\
                        2 * tf.matmul(tf.matmul(tf.transpose(self.Hsh_T), self.ph['Sim']), self.ph['H']) + \
                        tf.matmul(tf.matmul(tf.transpose(self.ph['H']), self.ph['Sim']), self.ph['H'])
        self.Loss_t2l = tf.trace(self.Loss_t2l) / 250000
        self.Loss_T_hsh = self.Loss_pair_t + self.Loss_t2l + self.mse_loss(self.Hsh_T, self.ph['B'])
    ...
    def train_img(self, learn_rate, update):
        ...
        for iter in tqdm(xrange(num_train / self.batch_size)):
            ...
            image = image - np.repeat(self.meanpix, self.batch_size, axis=0)
    ...
    def generate_code(self, Modal, bit, generate):
        ...
        if generate == "label":
            ...
            for iter in tqdm(xrange(num_data / batch_size + 1)):
                ind = index[iter * batch_size: min((iter + 1) * batch_size, num_data)]
                mean_pixel = np.repeat(self.meanpix, len(ind), axis=0)
                image = Modal[ind, :, :, :].astype(np.float32)
                image = image - mean_pixel.astype(np.float32)
